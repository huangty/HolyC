...
public class FlowSwitch extends BroadcastReceiver {
	/**
	 * Log name
	 */
	...
	public void onReceive(Context context, Intent intent) {
		if (intent.getAction().equals(HolyCIntent.OFPacketIn_Intent.action)) {
			byte[] ofdata = intent
					.getByteArrayExtra(HolyCIntent.OFPacketIn_Intent.data_key);
			int port = intent.getIntExtra(
					HolyCIntent.OFPacketIn_Intent.port_key, -1);
			...
			ByteBuffer bb;
			if ( ofm.getDataLayerType() == (short) 0x0806){ //handle arp sepcially
				bb = getARPResponse(out, opie, ofm, cookie);
				Log.d(TAG, "Forward to getARPResponse");
			}else{
